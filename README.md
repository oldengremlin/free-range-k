...
Опції:
  ...
  -G, --global                  Додати вкладку Global у --web дашборд (потрібно 2+ роутери)
...
> `FREE_RANGE_NO_COLOR`, `FREE_RANGE_DEBUG`, `FREE_RANGE_TABLE`, `FREE_RANGE_WEB`, `FREE_RANGE_GLOBAL` — вмикаються
> будь-яким непорожнім значенням (`true`, `1`, `yes` — всі спрацьовують).
...
# HTML-дашборд з глобальною вкладкою (SHARED-статус по всіх роутерах)
free-range -H r1,r2,r3 -s ukrhub.net -u admin -p secret -g /var/www/html --web -G
